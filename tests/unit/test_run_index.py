...
def test_run_index_skip_unchanged_file(tmp_path, mock_ctx):
    ...
    with patch('ingestion.pipeline.determine_strategy', return_value=ReindexStrategy.SKIP), \
         patch('ingestion.pipeline.logger') as mock_logger:
        run_index(str(f), mock_ctx)
        mock_logger.info.assert_called_with("Skipping indexing for %s: no changes detected", str(f))
...
def test_run_index_full_index_new_file(tmp_path, mock_ctx):
    ...
    with patch('ingestion.pipeline.determine_strategy', return_value=ReindexStrategy.FULL_INDEX), \
...
def test_run_index_purge_deleted_file(tmp_path, mock_ctx):
    ...
    mock_ctx.db.get_file_record.return_value = {"file_hash": "hash", "last_modified_timestamp": 123}
...
def test_run_index_metadata_update(tmp_path, mock_ctx):
    ...
    with patch('ingestion.pipeline.determine_strategy', return_value=ReindexStrategy.METADATA_UPDATE), \
         patch('ingestion.pipeline.run') as mock_run, \
         patch('ingestion.pipeline.logger') as mock_logger:
        run_index(str(f), mock_ctx)
        ...
        mock_ctx.db.update_file_metadata.assert_called_with(str(f), os.path.getmtime(str(f)))
